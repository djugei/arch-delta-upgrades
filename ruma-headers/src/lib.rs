...
mod rfc8187;
use http_headers::{
    is_tchar, is_token, quote_ascii_string_if_required, sanitize_for_ascii_quoted_string, unescape_string,
};
...
impl PartialEq<TokenString> for ContentDispositionType {
    fn eq(&self, other: &TokenString) -> bool {
        self.to_string().eq_ignore_ascii_case(&other.0)
    }
}
...
mod tests {
    ...
    fn parse_content_disposition_valid() {
        ...
        assert_eq!(content_disposition.disposition_type.to_string(), "custom");
        assert_eq!(content_disposition.filename, None);
        ...
        assert_eq!(content_disposition.disposition_type.to_string(), "form-data");
